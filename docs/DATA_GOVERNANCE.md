...
For Supabase deployment, apply all three migrations in order. `202607190002_customer_assignment_rls.sql` narrows the normalized customer tables. `202607190003_operational_workspace.sql` connects the current Imported Workspace using per-entity records, Auth-derived roles, assignment-aware RLS, Realtime and append-only audit inserts. Live two-organization and two-executive tests remain mandatory before real customer data is used.
