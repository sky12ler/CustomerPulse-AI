...
For Supabase deployment, apply both migrations in order. `202607190002_customer_assignment_rls.sql` removes the original tenant-wide read policy from customer-domain tables, defines `can_access_customer`, checks `assigned_profile_id = auth.uid()` for Account Executives, scopes related messages/analyses/components through their parent customer, and adds `customer_id` to audit records for reliable relevant-event policy. Live two-organization and two-executive tests are still mandatory because the current browser runtime is not connected to Supabase.
