...
Generate the binary artefacts with `node scripts/generate-mock-assets.mjs`. Reset configured Supabase with `supabase db reset`, then `node scripts/reset-demo.mjs --confirm`. Neither command deletes `/mock-data`.
