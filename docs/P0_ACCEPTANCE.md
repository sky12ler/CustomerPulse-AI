...
Verified 18 July 2026. “Browser verified” means exercised with Playwright against an optimized Next.js server. “Unit verified” means executed with Vitest. External services are not marked verified without credentials and a live response.
...
| Authorized production-compatible uploads   | Four-stage wizard, type RBAC, lazy parser runtimes; Playwright 2–4; all mock imports unit-tested        | Local browser/unit verified; production retest required after deployment |
...
| Tests/build/security                       | Lint, typecheck, 47 unit, 26 Playwright, build, audit, secret scan                                      | Passed                                                                   |
...
No OpenAI, Buffer, or Supabase credentials were available. Therefore live GPT-5.6 responses, Buffer delivery, and remote Supabase deployment are not claimed. Demo AVO and Demo Publisher are verified and explicitly labelled. Production Vercel verification is recorded separately only after the pushed `workflow-v2` release is observed.
